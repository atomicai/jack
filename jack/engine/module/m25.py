...
class BM25:
    def __init__(self, corpus, tokenizer=None, max_processes: int = None):
        self.corpus_size = 0
        self.avgdl = 0
        self.doc_freqs = []
        self.idf = {}
        ...
        self.max_processes = max_processes if max_processes else -1
...
class BM25Plus(BM25):
    ...
    def get_scores(self, query):
        ...
        for q in query:
            q_freq = np.array([(doc.get(q) or 0) for doc in self.doc_freqs])
            score += (self.idf.get(q) or 0) * (
                self.delta + (q_freq * (self.k1 + 1)) / (self.k1 * (1 - self.b + self.b * doc_len / self.avgdl) + q_freq)
            )
        return score
