...
from farm.data_handler.processor import BertStyleLMProcessor, Processor
from farm.modeling.tokenization import EmbeddingTokenizer
from farm.utils import log_ascii_workers
from jack.tooling import pic, tool
...
class Ignitor:
    ...
    def _get_dataset(self, filename, dicts=None):
        ...
        num_dicts = len(dicts)
        multiprocessing_chunk_size, num_cpus_used = tool.calc_chunksize(
            num_dicts=num_dicts,
        ...
        with ExitStack() as stack:
            if self.max_processes > 1:  # use multiprocessing only when max_processes > 1
                ...
                tool.log_ascii_workers(num_cpus_used, logger)
    ...
    def _load_data(self, train_dicts=None, dev_dicts=None, test_dicts=None):
        ...
        logger.info("\nLoading data into the data silo ..." "{}".format(pic.THEBAT))
        # train data
...
class SIgnitor:
    """
    S (`streaming`) Ignitor loads and preprocesses datasets in parallel to the model training.
...
class _StreamingDataSet(IterableDataset):
    ...
    def __iter__(self):
        ...
        dicts = tool.grouper(self.file_to_dicts_generator, n=10, worker_id=worker_id, total_workers=total_workers)
...
class СVIgnitor:
    ...
    def _make_question_answering(
        cls, datasilo, sets=["train", "dev", "test"], n_splits=5, shuffle=True, random_state=None, n_neg_answers_per_question=1
    ):
        """
        Create number of folds data-silo-like objects which can be used for training from the
    ...
    @staticmethod
    def _make(datasilo, sets=["train", "dev", "test"], n_splits=5, shuffle=True, random_state=None, stratified=True):
        """
        Create number of folds data-silo-like objects which can be used for training from the
        original data silo passed on.
    ...
    @staticmethod
    def _make_nested(
        datasilo, sets=["train", "dev", "test"], n_splits=5, shuffle=True, random_state=None, stratified=True, n_inner_splits=5
    ):
        setstoconcat = [datasilo.data[setname] for setname in sets]
        ds_all = ConcatDataset(setstoconcat)
        idxs = list(range(len(ds_all)))
