...
class TNode(object):
    ...
    def pyx_insert(self, node, key, value, priority):
        ...
        if node is None:
            ...
            node.key = key
            # TODO: here it should be Iterable ... double check it.
            assert isinstance(node.value, Iterable)
    ...
    def remove(self, node, key, value=None):
        '''Remove a node - just call the fast version'''
        return self.pyx_remove(node, key, value)

    def pyx_remove(self, node, key, value=None):
        ...
        if node is not None:
            if key < node.key:
                ...
                if node.left is None:
                    node.value.discard(value)
                    return (True, node.right) if len(node.value) <= 0 else (False, node)

                if node.right is None:
                    node.value.discard(value)
                    return (True, node.left) if len(node.value) <= 0 else (False, node)

                # Here we apply rotation(s) to preserve the property of heap
                if node.left.priority < node.right.priority:  # Inner node => both exist
                    node = self.rotate_with_left_child(node)
                else:
                    node = self.rotate_with_right_child(node)
    ...
    def remove_min(self, node, value=None):
        '''Remove the lowest node below us'''
        if not (node is None):  # Tuple's Wrapping allow us to faster iterating
            if not (node.left is None):
                (node.left, result) = self.remove_min(node.left, value)
            else:
                # Minimum found
                # TODO: Hijack here to remove node iff it contains value
                node.value.discard(value)
                if len(node.value) <= 0:
    ...
    def remove_max(self, node, value=None):
    ...
    def rotate_with_left_child(self, node):
        # TODO:
        # Обрати внимание! При пробрасывании сюда узла - он пробрасывается копируя ссылку и
        # и в любом из поворотов указывает на "верхний" (корень) узел, который мы хотим повернуть
        # Но после поворота мы возвращем ссылку на другой узел (нижний)
        # Кроме того, самое важное в том, что она присваивается самому узлу
        # т.е. (1) node.left = self.insert(node.left, key, value, priority)
        # Справа в (1) вернется новый узел (после поворота). Который потом, в свою очередь,
        # будет присвоен ссылке откуда был вызван
        # node.left = ...
        '''This is a treap thing - rotate to rebalance'''
        x = node.left  # x
        node.left = x.right  # set y's left subtree to the x's right subtree
        x.right = node
        node = x
        return node
...
class Treap(object):
    ...
    def __setitem__(self, key, value, priority=None):
        ...
        if self.root is None:
            ...
            assert length_delta in [0, 1]  # Whether new node is created or not.
    ...
    def remove_min(self, value=None):
        '''Remove the lowest node in the treap'''
        if self.root is not None:
            (self.root, result) = self.root.remove_min(self.root, value)
    ...
    def predecessor(self, node, value=None):
    ...
    def successor(self, node, value=None):
